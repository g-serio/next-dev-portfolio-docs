---
title: "Environment Variables for next-dev-portfolio"
...
When neither `NEXT_PUBLIC_OLONJS_CLOUD_URL` nor its legacy alias is configured, the site runs in Local mode.
...
### Save2Repo (`SAVE2REPO=true` + cloud variables)

When `NEXT_PUBLIC_OLONJS_SAVE2REPO` is `true` and cloud variables are configured, the site runs in Save2Repo mode.
...
When cloud variables are configured but `NEXT_PUBLIC_OLONJS_SAVE2REPO` is not `true`, the site runs in Live mode.
