...
Open `theme.json` and you'll find a `tokens` object with four top-level sections: `colors`, `typography`, `borderRadius`, `spacing`, and a nested `modes` object for light-mode overrides.
...
The `tokens.spacing` object controls three layout-level dimensions that affect the overall density and rhythm of every page.
...
```json
{
  ...
  "tokens": {
    "colors": {
      ...
      "muted": "#151515",                // secondary panel backgrounds
